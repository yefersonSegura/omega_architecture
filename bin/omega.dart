...
- FORBIDDEN (analyzer: abstract class can't be instantiated): OmegaEventName('navigation.intent') or OmegaIntentName('navigate.register') — [OmegaEventName] and [OmegaIntentName] are abstract contracts only. You MUST pass a concrete enum value, e.g. OmegaEvent.fromName(AppEvent.navigationIntent, payload: OmegaIntent.fromName(AppIntent.navigateLogin)). Prefer `with OmegaEventNameDottedCamel` / `OmegaIntentNameDottedCamel` on **module** enums (camelCase members → dotted `.name`). Explicit `const Case('wire')` + `final String name` is only for rare **app_semantics.dart** app-wide enums — **never** use that hand-wired style in `lib/<module>/*_events.dart` (use DottedCamel only).
...
- **FORBIDDEN in `*_events.dart` (feature modules):** hand-wired “BLoC style” enums — `enum FooIntent implements OmegaIntentName { bar('a.b'); const FooIntent(this.name); @override final String name; }` / the same for `OmegaEventName`. That pattern is **not** the module template; it duplicates literals the mixin already derives and confuses `OmegaIntent.fromName` / behavior `ctx.intent?.name` checks. **Exception:** editing **`lib/omega/app_semantics.dart`** for app-wide `AppIntent` / `AppEvent` may use explicit constructors — do **not** copy that style into `lib/<module>/*_events.dart`.
