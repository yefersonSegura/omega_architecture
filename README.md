...
**How the web docs are published:** [`.github/workflows/docs.yml`](.github/workflows/docs.yml) builds VitePress and deploys with **`upload-pages-artifact`** + **`deploy-pages`** (same approach as [omega-workspace](https://github.com/yefersonSegura/omega-workspace)). **Settings → Pages** → **Build and deployment** → source **GitHub Actions** (not “Deploy from a branch” unless you only push to `gh-pages` yourself). If Pages is set to **main** + **`/docs`**, GitHub serves Markdown sources without a built **`index.html`** → *“File not found … provide an index.html”*. Use **`https://yefersonsegura.github.io/omega_architecture/`** (repo path in the URL).
