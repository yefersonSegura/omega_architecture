...
**How the web docs are published:** [`.github/workflows/docs.yml`](.github/workflows/docs.yml) pushes the VitePress **`dist`** (with **`index.html`** at the root of that output) to branch **`gh-pages`**. **Settings → Pages** must be **Deploy from a branch** → **`gh-pages`** → **`/ (root)`**. If Pages is set to **main** + **`/docs`**, GitHub looks for **`docs/index.html`** in the repo, but this project only has **`docs/index.md`** (sources); the built **`index.html`** lives under **`docs/.vitepress/dist/`** and is what gets pushed to **`gh-pages`** — wrong Pages folder causes *“File not found … provide an index.html”*. After each run, see the log **“Push dist to gh-pages”** and the **Summary** tab. **Settings → Actions → General** → **Read and write**. Open **`https://yefersonsegura.github.io/omega_architecture/`** (with the repo path), not only `https://yefersonsegura.github.io/`.
